    ...
    for i,dict in enumerate(param_dict):
        for j,style in enumerate(dict['style']): # Loop up branch
            branch[i] = lasagne.layers.Conv3DLayer(
                ...
                name = block_name+'_'+str(i)+'_'+str(j)) if style=='convolutional' else lasagne.layers.NonlinearityLayer(lasagne.layers.Pool3DLayer(
                incoming=incoming if j == 0 else branch[i],
...
def get_model():
    ...
    l_pool = BN(lasagne.layers.Pool3DLayer(incoming=l_conv17, pool_size=(4,4,4), mode='average_inc_pad'))    
    
    
    
    
    #Fully-Convolutional-Layer
    l_score = lasagne.layers.Conv3DLayer(
        incoming = l_pool,
        num_filters = 2,
        filter_size=(1,1,1)
        )
